...
First, do some exploratory visualization, mapping species onto point color. Does it look like there is an opportunity to cluster by species?
...
Here we use the NbClust::NbClust() function, which "provides 30 indices for determining the number of clusters and proposes to use the best clustering scheme from the different results obtained by varying all combinations of number of clusters, distance measures, and clustering methods." See ?NbClust for more information. 

Basically, it is going to run 30 different ways of evaluating how many clusters it thinks exist, then tell you the breakdown of what they decide (e.g. "8 algorithms think that there should be 4 clusters").
...
```{r}
# By these estimates, 2 is identified as the best number of clusters by the largest number of algorithms (8/30)...but should that change our mind? Maybe...but I think it makes sense to still stick with 3 (a cluster for each species) and see how it does. 
```

### Create a complete, scaled version of the data

We're still going to use 3 clusters and see how it does, though there may be a case here for 2 given that adelie and chinstrap penguins are pretty similar. We are going to do this with complete cases - in other words, for the variables we're using to perform k-means clustering on penguins (bill length, bill depth, flipper length, body mass), we are dropping any observation (row) where any of those are missing. Keep in mind that this may not be the best option for every scenario - in other cases (e.g. when we have a large proportion of missingness), we may want to input missing values instead. 
...
# I think what scale does is that the median or average for each column and then fills each observation as a plus or minus from that value. So if orginally an obs was 25 but the "center" values was 26.5, then the scale would by -1.5?
```

### Run k-means

Now that we have complete, scaled data for the 4 size variables of interest, let's run k-means. You should know the iterative process it's running through from the Week 8 lecture.
...
# 87 obs assigned to cluster 1, 132 obs assigned to cluster 2, 123 obs assigned to cluster 3
```
...
Here, we'll read in the WorldBank environmental data (simplified), and keep only the top 20 GHG emitters for the dataset
...
# Only keep the top 20 greenhouse gas emitters (for simplifying visualization here...)
...
```{r}
# Scale the numeric variables (column 3:7)
wb_scaled <- wb_ghg_20 %>% 
  select(3:7) %>% 
  scale()
...
# Check out the outcome with View(wb_scaled) - see that the rownames are now the country name (this is useful for visualizing)
```

Great, now we have simplified, scaled version of the numeric variables, with rownames containing the county name.
...
Here's how you can make your dendrogram in ggplot (here, I'll use the complete linkage example stored as hc_complete) using ggdendgrogram(), a ggplot wrapper:
...
Cool, then you customize with usual ggplot tools. 
